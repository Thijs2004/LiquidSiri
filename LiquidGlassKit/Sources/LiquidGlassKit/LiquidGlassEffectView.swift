...
public class LiquidGlassEffectView: UIView, AnyVisualEffectView {
    ...
    public required init(effect: LiquidGlassEffect) {
        ...
        var lg = effect.style.liquidGlass
        lg.fullQuality = effect.fullQuality
        if let tint = effect.tintColor {
            lg.tintColor = tint
        }

        let liquidGlassView = LiquidGlassView(lg)
    ...
    /// Synchronously capture the current background content.
    /// Call this before making the view visible for the first time (e.g. inside an
    /// asyncAfter block, right before un-hiding) so the very first rendered frame
    /// already shows correct content — eliminates the "readjustment" flash.
    @discardableResult
    public func captureBackground() -> Bool {
        liquidGlassView?.captureBackground() ?? false
    }
}
...
public class LiquidGlassEffect: UIVisualEffect {
    ...
    public enum Style {
        case regular, clear, clearBlur, regularHighBlur, transparent, noCapture, clockGlass

        @available(iOS 26.0, *)
        var nativeStyle: UIGlassEffect.Style {
            switch self {
            case .regular, .regularHighBlur: .regular
            case .clear, .clearBlur, .transparent, .noCapture, .clockGlass: .clear
            }
        }
        ...
        var liquidGlass: LiquidGlass {
            ...
            case .regularHighBlur: .regularHighBlur
            case .clear: .clear
            case .clearBlur: .clearBlur
            case .transparent: .transparent
            case .noCapture: .noCapture
            case .clockGlass: .clockGlass
            }
        }
    }
    let style: Style
    ...
    /// Whether to use high-quality rendering (no resolution scaling).
